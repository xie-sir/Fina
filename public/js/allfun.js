...
function showlist(data) {
	...
	htmlstr = '<li class="divider"></li><li><a href="javascript:;" onclick="sharego();">';
    $('#userlist').append(htmlstr);
    $('#nowinline').html('在線' + count + '人 <span class="caret"></span>');
    if (self != top) {
        if (getParamValue('wall') == 1) {
            if (count > 1) {
                $('#sharefq').hide()
...
function sendsay(uid) {
    ...
    var txt = prompt("請輸入您要對" + tmpname + "說的話", "");
    if (txt) {
        socket.emit('say msgs', {id: uid, say: txt});
        var tmptou = '對[大家]說：';
        if (uid)
            tmptou = '悄悄对對' + tmpname + '說：';
        say('(我)' + tmptou + txt)
    }
}
